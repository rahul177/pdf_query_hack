...
class Config(BaseModel):
    ...
    langfuse_host: str = "http://localhost:3000"  # Corrected default port
    openai_api_key: str = ""
...
class ElementSummarizer:
    def __init__(self):
        ...
        self.prompt_template = ChatPromptTemplate.from_template(
            """Summarize the following {element_type} content while preserving key details, 
            relationships, and context. Focus on numerical data, labels, and relationships 
            between elements for tables and diagrams. Write a concise summary that captures the essence
            of the content without losing important information. Clearly summarize the content number in table carefully
    ...
    def summarize(self, element: Dict[str, Any]) -> Dict[str, Any]:
        if element["type"] in ["table", "image"]:
            trace = langfuse.trace(name="element_summarization") if langfuse else None
            generation = trace.generation(
                name="summarize_element",
                input={"element_type": element["type"], "element_content": element.get("text", "")[:1000]},
            ) if trace else None
            ...
            summary = self.chain.invoke(
                {"element_type": element["type"], "element_content": content},
                config={"callbacks": [CallbackHandler()]} if langfuse else {}
            )
...
class PDFIndexer:
    ...
    def index_documents(self, documents: List[Document], collection_name: str) -> Dict[str, Any]:
        ...
        embedder = HuggingFaceEmbeddings(model_name=config.text_embedding_model)
...
class QueryProcessor:
    def __init__(self):
        self.llm = ChatOpenAI(model=config.chat_model, temperature=0, api_key=config.openai_api_key)
        self.embedder = HuggingFaceEmbeddings(model_name=config.text_embedding_model)
    ...
    def _init_prompts(self):
        self.query_translation_prompt = ChatPromptTemplate.from_template(
            """Translate the user's natural language query into effective search terms.
            Consider synonyms, technical terms, and domain-specific terminology.
        ...
        self.query_expansion_prompt = ChatPromptTemplate.from_template(
            """Expand the search query to include related terms and concepts.
            Generate 3 query variations that might help retrieve relevant documents.
            Don't include numbers or bullet points, just plain text.
        ...
        self.query_router_prompt = ChatPromptTemplate.from_template(
            """Classify the query type and determine the best search strategy weights.
            Respond with JSON format with weights for keyword, vector, and knowledge_graph.
            
            Query types:
            - Factual: Simple facts (weight keyword higher)
            - Conceptual: Abstract concepts (weight vector higher)
            - Relational: Connections between concepts (weight knowledge_graph higher)
    ...
    # First, modify the QueryProcessor's process_query method to return more detailed information
    ...
    # Then modify the retrieve_documents method to handle the new format
    ...
    def _expand_query(self, query: str) -> List[str]:
        ...
        return [query] + [x.strip() for x in expansions.split("\n") if x.strip()]
    ...
    def _vector_search(self, client: QdrantClient, query: str, collection_name: str, k: int) -> List[Document]:
        ...
        return [
            Document(
                ...
                metadata={
                    **hit.payload["metadata"],
                    "score": hit.score,  # Qdrant returns this by default
                    "retriever": "Vector"
                }
            )
            for hit in results
    ...
    def _deduplicate_and_rerank(
        ...
        scored_docs = list(zip(unique_docs.values(), scores))
...
class ResponseGenerator:
    def __init__(self):
        self.llm = ChatOpenAI(model="gpt-4", temperature=0.3, api_key=config.openai_api_key)
        self.prompt = ChatPromptTemplate.from_template(
            """You are an expert assistant analyzing PDF documents. Use the following context to answer the question.
            The context includes text chunks, figures, and tables from the document.
            ...
            Provide a detailed answer with references to specific sections, figures, or tables when relevant.
            If you don't know the answer, say you couldn't find the information in the document.
        ...
        if question:
            with st.spinner("Analyzing your question..."):
                try:
                    ...
                    # Show query information in a table
                    st.subheader("Query Analysis")
                    
                    # Translated query
                    st.markdown(f"**Translated Query:** {translated_query}")
