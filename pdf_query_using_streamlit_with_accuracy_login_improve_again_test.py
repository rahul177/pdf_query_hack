...
def show_pdf(file_path, page_num, highlight_text=None):
    doc = get_pdf_document(file_path)  # This will use cached version
...
class RetrieverEvaluator:
    def __init__(self):
        self.metrics = {
            ...
            "recall": None,
            "f1": None,
            "mrr": None  # Mean Reciprocal Rank
        }
        self.sample_data = None
        self.eval_dataset = None
    
    def load_natural_questions(self, sample_size=50):
        """Load Natural Questions dataset from HuggingFace"""
        trace = start_trace("load_natural_questions")
        try:
            # Load a subset of the dataset
            dataset = load_dataset("natural_questions", split=f"train[:{sample_size}]")
            
            # Process into our format
            processed_data = []
            for item in dataset:
                # Get the first answer if available
                answer = item['annotations']['short_answers'][0]['text'][0] if item['annotations']['short_answers'] else ""
                
                processed_data.append({
                    "question": item['question']['text'],
                    "answer": answer,
                    "context": item['document']['html']
                })
            
            self.eval_dataset = pd.DataFrame(processed_data)
            if trace:
                trace.update(
                    output={"status": "loaded"},
                    metadata={"sample_size": len(self.eval_dataset)}
                )
    ...
    def evaluate_with_indexer(self, indexer: PDFIndexer, query_processor: QueryProcessor):
        """Full evaluation pipeline with our actual indexer and retriever"""
        trace = start_trace("full_evaluation")
        try:
            if self.eval_dataset is None:
                self.load_natural_questions()
            
            # Create a temporary PDF for evaluation
            with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tmp:
                pdf_path = tmp.name
            
            # Convert HTML context to PDF (simplified - in real app use proper HTML-to-PDF)
            self._html_to_pdf(self.eval_dataset['context'].iloc[0], pdf_path)
            
            # Index the document
            elements = indexer.parse_pdf(pdf_path)
            documents = indexer.process_elements(elements)
            collection_name = f"{config.qdrant_collection_prefix}eval_{str(uuid.uuid4())[:8]}"
            ...
            for _, row in self.eval_dataset.iterrows():
                ...
                queries_with_weights, _ = query_processor.process_query(question)
                retrieved_docs, _ = query_processor.retrieve_documents(
                    queries_with_weights=queries_with_weights,
                    collection_name=collection_name,
                    top_k=10
                )
                
                # Check if answer appears in top documents
                found = any(answer.lower() in doc.page_content.lower() for doc in retrieved_docs)
                results.append({
                    "question": question,
                    "answer": answer,
                    "found": found,
            ...
            self.metrics = {
                ...
                "recall": true_positives / len(results),  # Same as precision in this simple case
                "f1": true_positives / len(results),
                "mrr": self._calculate_mrr(results),
                "true_positives": true_positives,
                "false_positives": 0,  # Not tracking in this simple version
                "false_negatives": false_negatives,
                "coverage": true_positives / len(results)
            }
            
            if trace:
                trace.update(
                    output=self.metrics,
                    metadata={"questions_evaluated": len(results)}
                )
            
            # Clean up
            os.unlink(pdf_path)
            return self.metrics, pd.DataFrame(results)
    ...
    def _calculate_mrr(self, results):
        ...
        for result in results:
            if result['found']:
                # Simplified - assume first doc is rank 1 if found
                ranks.append(1)
        return 1.0 / np.mean(ranks) if ranks else 0
    
    def _html_to_pdf(self, html_content, output_path):
        """Simplified HTML to PDF conversion - in production use proper library"""
        import pdfkit
        try:
            ...
            doc = fitz.open()
            page = doc.new_page()
            page.insert_text((50, 50), html_content[:5000])
...
def main_application():
    ...
    with tab2:
        ...
        st.markdown("Evaluate the retriever's performance using Natural Questions dataset")
        
        if "evaluator" not in st.session_state:
            st.session_state.evaluator = RetrieverEvaluator()
        
        sample_size = st.slider("Number of questions to evaluate", 5, 100, 20)
        
        if st.button("Run Full Evaluation"):
            with st.spinner("Running comprehensive evaluation..."):
                try:
                    # Load the dataset
                    st.session_state.evaluator.load_natural_questions(sample_size)
                    
                    # Run evaluation with our actual components
                    metrics, detailed_results = st.session_state.evaluator.evaluate_with_indexer(
                        indexer=PDFIndexer(),
                    ...
                    st.markdown(f"""
                    - **Answer Coverage**: {metrics['coverage']:.2%} of questions had answers in retrieved documents
                    - **Average Documents Retrieved**: {detailed_results['retrieved_docs'].mean():.1f} per question
                    """)
                    ...
                    for i, row in detailed_results.head(3).iterrows():
                        with st.expander(f"Q: {row['question']}"):
                            ...
                            st.markdown(f"**Found in results:** {'✅' if row['found'] else '❌'}")
